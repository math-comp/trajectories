...
\item The algorithm will naturally produce cells where the two events
  ...
  cells,
\item When building trajectories, make sure to invalidate doors corresponding
...
is simply a second-degree multivariate polynomial.  These computations can
...
of a closed cell.  It is later used when computing trajectories.  One
...
data that is stored for later use.
...
replaced by points with a lower approximation or a higher approximation
...
a point is given either as a pair of coordinates or the pair of a first
coordinate and an obstacle in which we take the point with that first
coordinate, so that the information is processed in the right way by
the following algorithms.
\subsection{Concerning the neighboring relation}
In our use case to compute trajectories, we need to know which are the
...
intersect, perform the needed replacement and add the intersection as a
...
help to prove this improved algorithm with optimal reuse of the proofs
...
approximation as extremity are close enough to the original obstacles
...
the scene description.  If the scene description was drawn by a
