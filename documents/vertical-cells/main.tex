...
\item In the second level, we add properties that fall in three categories
  ...
  is the position of the left side of the last open cell.
...
At each event, the sequence of closed cells evolves.  In the general
case, new closed cells are created from the open cells in contact, and we
need to prove that the invariant properties satisfied by the open cells
are sufficient to guarantee that the created closed cells are well formed.

In our work, we rely quite a lot on a relation between edges, named
{\tt edge\_below} and noted by the infix symbol {\tt <|}.  It turns
...
sequence of edges is sorted for the {\tt edge\_below} relation ({\tt <}).  We
...
All open cells meet the current position of the vertical edge.  As a
...
the intersection of vertical cylinders above the two edges.

An edge \(g\) is covered by the sequence of closed cells if there exists a
sequence of cells \(s\) with the following properties:
\begin{itemize}
\item all edges in \(s\) have \(g\) as their high edge,
\item each cell's right side is on the same vertical as the next cell's
  left side,
\item the first cell's left side is aligned with the edge's left side,
\item the last cell's right side is aligned with the edges right side.
\end{itemize}
Some edges are only partially covered by the closed cells, but in that
case their right side is covered by an open cell.  In that case, there
is a sequence \(s\) and an open cell \(c_o\) such that:
\begin{itemize}
\item all edges in \(s \cup \{c_o\}\) have \(g\) as their high edge,
\item for the sequence \(s.c_o\) each cell's right side is on
  the same vertical as the next cell's left side,
\item the left side of the first cell in \(s\)
  is aligned with the edge's left side,
