...
\item Stop each time one meets an event (e.g. an edge tip)
\item maintain a vertically ordered sequence of incomplete cells
\begin{itemize}
\item Complete all incomplete cells in contact with the event
\item Create new incomplete cells forall edges starting at this event
\end{itemize}
...
\begin{itemize}
\item Maintain a sequence of incomplete cells
\begin{itemize}
\item In code and the article, they are called ``open cells''
\end{itemize}
\item Compute incomplete cells in contact with the current event
\item Complete these cells
\item Create new incomplete cells starting at the current event
\end{itemize}
\end{frame}
\begin{frame}
...
\item Incomplete cells are green, pink, grey (ascending order)
\item Contact cell: the pink cell
\item New complete cell: complete the pink cell at the event,
obtain a dark green cell in the middle
\item New incomplete cells: light purple and yellow
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
\item  Width of complete cells : horizontal distance between events
\begin{itemize}
...
\begin{itemize}
\item Keep track of last created incomplete and complete cells
...
\begin{itemize}
\item All cells have a high, a low edge, and a left point sequence
\item Incomplete cells have no right points
\item Point sequences go from the high edge to the low edge
\item Intervals between successive points in sequences describe safe doors
\end{itemize}
\end{frame}
\begin{frame}
...
\end{frame}
\begin{frame}
\frametitle{Non-vertically aligned events: new incomplete cells}
\begin{columns}
\begin{column}{0.3\textwidth}
%trim parameters left bottom right top
%\framebox
...
\begin{column}{0.7\textwidth}
\begin{itemize}
\item Two new incomplete cells are created (in this case)
...
\begin{column}{0.7\textwidth}
\begin{itemize}
\item Cell \(c_5\) is completed when processing \(e'\)
...
\item Producer code must guarantee this
\item Consuming code maintains it easily
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\item 3 fields compose the sequence of incomplete cells
\begin{itemize}
\item Direct link to the last created incomplete cell (\(c_5\))
\item Two other fields for prefix and suffix 
\end{itemize}
\item 2 fields for the set of complete cells
\begin{itemize}
\item Direct access to the last created complete cell (\(c_3\))
...
\end{frame}
\begin{frame}
\frametitle{Invariants of incomplete cell sequences}
\begin{itemize}
...
\begin{itemize}
\item Incomplete cells are disjoint
\item Incomplete cells are disjoint from complete cells
\item complete cells are disjoint
\item Obstacles are progressively included in the top of all cells
\end{itemize}
\end{frame}
\begin{frame}
\frametitle{Future improvements}
\begin{itemize}
\item Remove constraints of edges not crossing
\begin{itemize}
\item Revisit the proof to remove uses of {\tt edge\_below}
\item Detect edge crossings incrementally
\end{itemize}
\item Add a field to cells to point to the neighbors
\item Understand where efficient numbers can be used
\begin{itemize}
\item For now rational numbers, hope to use floating point numbers
\end{itemize}
\end{itemize}
\end{frame}
\end{document}
