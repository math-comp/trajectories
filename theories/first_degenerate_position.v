...
Lemma update_open_cell_edge_covered_non_gp_invariant
  ...
  lstx = p_x (point ev) ->
  (point ev) <<< lsthe ->
  edge_covered_non_gp_invariant bottom top s past
     (Bscan fop lsto lop cls lstc lsthe lstx)
     (ev :: evs) ->
  edge_covered_non_gp_invariant bottom top s (rcons past ev)
     (step (Bscan fop lsto lop cls lstc lsthe lstx) ev)
    evs.
Proof.
...
have d_inv' :
  disjoint_non_gp_invariant bottom top s
    (Bscan (fop ++ nos) lno lop cls
      (update_closed_cell lstc (point ev)) lsthe (p_x (point ev))) evs.
...
rewrite /= inE=> /orP[/eqP ->| ].
  ...
  rewrite /in_safe_side_left /= /left_limit /= xlst !inE !negb_or.
  move=> /andP[] /[dup] /eqP xp -> /andP[] puf /andP[] -> /andP[] ->.
  rewrite /= andbT.
  ...
  rewrite -catA -cat_rcons !mem_cat orbCA orbC=> /orP[cold | cnew].
    ...
    have xp : p_x p = p_x (point ev).
      have xpl : p_x p <= p_x (point ev).
        ...
        by rewrite /lexPt -bottom_left_x=> /orP[/ltW // |/andP[] /eqP -> _].
    ...
    (* TODO: probably needs a duplication here. *)
