...
Lemma hullX (F : @convType R) (C : set E) (D : set F) :
  hull (C `*` D) = hull C `*` hull D.
Proof.
...
rewrite Convn_pair/comp/=; congr pair;
    apply: (S1_inj R); rewrite [LHS]S1_Convn [RHS]S1_Convn big_prod_ord/=.
  apply eq_big => // i _.
  rewrite -(scale1pt (scalept _ _)) scaleptA // -(FDist.f1 e).
  rewrite mulr_suml.
  pose h x := e x * d i.
  have h0 x : 0 <= h x by rewrite /h mulr_ge0.
...
have -> : (\sum_(a in {: 'I_n * 'I_m} |
   ...
   {: 'I_n * 'I_m} | a \in pred1 (i, j))
       (FDist.f (fdist_prod d (fun=> e))) a)%R.
   apply: eq_big=>// k; congr andb; rewrite 3!inE.
   by apply: (eqtype.inj_eq _ k (i, j)); exact (can_inj (@unsplit_prodK _ _)).
...
#[export] HB.instance Definition _ := isAffine.Build _ _ _ _ linear_is_affine.
...
Lemma is_convex_set_preimage [T U : convType R] (f : {affine T -> U}) (A : {convex_set U}) :
...
Definition cone0 (A : set E) :=
  ([set t%:num *: a | t in (@setT {posnum R}) & a in A] `<=` A)%classic.
...
move=>Acone; split=>Aconv.
   move=>x [u uA] [v vA] <-.
   have uA2: A (2 *: u).
     by apply: Acone => /=; exists 2%:pos => //; exists u.
   have vA2: A (2 *: v) by apply Acone; exists 2%:pos =>//; exists v.
...
move=> t1; apply Aconv; exists ((Prob.p t : R) *: x);
  ...
  by exists (PosNum t0) =>//; exists x.
by exists (PosNum t1)=>//; exists y.
Qed.

(* Note: cone0_of A is NOT pointed due to lemma cone0_of_convE. *)
(* TODO: maybe change the 0 <= k i to 0 < k i in the definition of conv. *)

Definition cone0_of (A: set E) : set E :=
  [set a | exists n (s : 'I_n.+1 -> E) (k: 'I_n.+1 -> {posnum R}),
    \sum_i (k i)%:num *: (s i) = a /\ (range s `<=` A)%classic].

Lemma cone0_of_cone0 (A: set E): cone0 (cone0_of A).
Proof.
move=>x [t /= _] [a [n [s [k [<- sA]]]]] <-.
rewrite scaler_sumr; exists n, s, (fun i => (t%:num * (k i)%:num)%:pos); split => //.
by apply congr_big=>// i _; apply /esym; apply scalerA.
Qed.

Lemma cone0_of_hullE (A : set E) :
  cone0_of A = [set t%:num *: a | t in (@setT {posnum R}) & a in (hull A)]%classic.
Proof.
rewrite eqEsubset; split=>x.
   move=>[n [s [k [<- kA]]]]; set t := \sum_i (k i)%:num.
   have k0' (i : 'I_n.+1) : true -> 0 <= (k i)%:num by move=> _; apply/ltW.
   have: 0 <= t by apply sumr_ge0.
   rewrite le0r=>/orP; case.
      move=>/eqP /psumr_eq0P; move=> /(_ k0') /(_ ord0 Logic.eq_refl) /eqP.
      by rewrite gt_eqF.
   move=>t0.
   have tk0 i : (Rdefinitions.IZR BinNums.Z0 <= [ffun i => t^-1 * (k i)%:num] i).
      by rewrite ffunE; apply/mulr_ge0; [ apply ltW; rewrite invr_gt0 | apply k0' ].
   have tk1 : \sum_(i < n.+1) [ffun i => t^-1 * (k i)%:num] i = 1.
      transitivity (\sum_(i < n.+1) t^-1 * (k i)%:num).
         by apply congr_big=>// i _; rewrite ffunE.
      rewrite -mulr_sumr mulrC divff//.
      by move:t0; rewrite lt0r=>/andP[].
   move:(t0)=> t0'; exists (PosNum t0')=>//; exists (t^-1 *: \sum_i (k i)%:num *: s i).
      exists n.+1, s, (@FDist.make _ _ (finfun (fun i=> t^-1 * (k i)%:num)) tk0 tk1); split=> //.
...
have ->: \sum_(i | true && ~~ (0 < d i)) t%:num *: (d i *: s i) = \sum_(i | true && ~~ (0 < d i)) 0 *: 0.
...
simple refine (ex_intro _ _ _).
   move=> j.
   apply: (fun x : {posnum R} => (t%:num * x%:num)%:pos).
   simple refine (PosNum _).
...
have fle: (Prob.p t)%:E * f u + (onem (Prob.p t))%:E * f v <= f (u <|t|> v).
   ...
   apply: leeD; rewrite (@lee_pmul2l R)//= lte_fin.
   by rewrite subr_gt0.
